...
def test_net(name,
    ...
    for i in range(0, num_images):
        ...
        if visualize and np.random.rand() < 0.01:
            ...
            print('Visualizing')
            im_name = 'test_iteration_' + str(i+1) + '_step_' + str(step)
            im2show = cv2.cvtColor(im2show, cv2.COLOR_RGB2BGR)
            im_show = torch.from_numpy(im2show).permute(2,0,1)
            print('Logging test images to tensorboard')
            logger.add_image(im_name, im_show)
